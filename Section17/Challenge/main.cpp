...
void fill(std::vector<std::shared_ptr<Test>> &numbers, int num) {

    int value {};
    for (int i = 0; i < num; i++) {
        std::cout << "Enter the value for [" << i +1 << "]: ";
        std::cin >> value;
        numbers.push_back(std::make_shared<Test>(value));
    }
}

void display(const std::vector<std::shared_ptr<Test>> &numbers) {
